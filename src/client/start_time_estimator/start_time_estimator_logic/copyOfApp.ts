let allBracketSizes = [4, 4, 4, 4];
let averageLengthOfAMatch = 5;
let sizeOfLargestRoundRobinBracket = 5;
let numberOfMatsAtEvent = 1;
let lengthOfEachTournamentRound = 10;
...
let determineNumberOfMatchesInEachBracketRoundForDoubleElimination = (
  ...
  console.log("here");
  console.log(numberOfMatchesInEachRound);
  return numberOfMatchesInEachRound;
};

//ask for Harry's TS help.
let determineNumberOfMatchesInEachBracketRoundForRoundRobin = (
  bracketSize: number
): Array<number> => {
  let numberOfRoundsRequired: any;
  let numberOfMatchesPerRoundRobinRound: any;
  if (bracketSize % 2 == 1) {
    numberOfRoundsRequired = bracketSize;
    numberOfMatchesPerRoundRobinRound = (bracketSize - 1) / 2;
  }
  if (bracketSize % 2 == 0) {
    numberOfRoundsRequired = bracketSize - 1;
    numberOfMatchesPerRoundRobinRound = bracketSize / 2;
  }
  //wtf????
  let numberOfMatchesInEachRound: Array<number> = [];
  for (let x = 0; x < numberOfRoundsRequired; x++) {
    numberOfMatchesInEachRound.push(numberOfMatchesPerRoundRobinRound);
  }
  console.log(numberOfMatchesInEachRound);
  return numberOfMatchesInEachRound;
};
...
let arrayOfBracketsWhereEachBracketIsAnArrayOfTheNumberOfMatchesRequiredThatBracketRound = (
  ...
  sizeOfLargestRoundRobinBracket: number
): Array<Array<number>> => {
  let finalArray: Array<Array<number>> = [];

  for (let x = 0; x < arrayOfBracketSizes.length; x++) {
    if (arrayOfBracketSizes[x] > sizeOfLargestRoundRobinBracket) {
      finalArray.push(
    ...
    if (arrayOfBracketSizes[x] <= sizeOfLargestRoundRobinBracket) {
      finalArray.push(
        determineNumberOfMatchesInEachBracketRoundForRoundRobin(
          arrayOfBracketSizes[x]
        )
      );
    }
...
let optimalRoundSpacingFunction = (
  ...
  > = arrayOfBracketsWhereEachBracketIsAnArrayOfTheNumberOfMatchesRequiredThatBracketRound(
    arrayOfBracketSizes,
    sizeOfLargestRoundRobinBracket
  );
